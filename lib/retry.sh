...
retry_run() {
    ...
    if [ $# -eq 0 ]; then
        error "retry_run: no command specified"
        return 1
    fi
    ...
    while [ "$attempt" -le "$max_attempts" ]; do
        ...
        if [ "$attempt" -lt "$max_attempts" ]; then
            warn "Attempt $attempt failed with exit code $exit_code, retrying in ${delay}s..."
            sleep "$delay"
            delay=$((delay * 2))
...
retry_diskutil() {
    ...
    while [ "$attempt" -le "$max_attempts" ]; do
        ...
        if grep -q "Resource busy\|busy" "$stderr_file" 2>/dev/null; then
            warn "diskutil: resource busy, attempting unmount"
            diskutil unmount "$@" 2>/dev/null || true
            sleep 3
            attempt=$((attempt + 1))
            continue
        fi

        if grep -q "IOKit\|Disk object not found" "$stderr_file" 2>/dev/null; then
            warn "diskutil: transient IOKit error, retrying"
